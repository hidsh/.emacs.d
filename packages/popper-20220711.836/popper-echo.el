...
(defcustom popper-echo-prompt "Popups: "
  ""
  :type 'string
  :group 'popper)

(defcustom popper-echo-prompt-group-format "Group (%%s): "
  ""
  :type 'string
  :group 'popper)

;; (propertize ", " 'face 'popper-echo-area-buried)
(defcustom popper-echo-delimiter ", "
  ""
  :type 'string
  :group 'popper)
                     ...
                     (cons
                      ...
                      (cl-mapcar (lambda (key buf)
                                   (concat
                                    ;; mod
                                    ;; (propertize ", " 'face 'popper-echo-area-buried)
                                    (propertize popper-echo-delimiter 'face 'popper-echo-area-buried)
                                    (when key
                                      (concat
                                       (propertize "[" 'face 'popper-echo-area-buried)
    ...
    (let* ((max-width (- (* popper-echo-lines (frame-width))
      ...
      (message "%s"
               (concat
                ...
                ;; (if group (format "Group (%s): " group) "Popups: ")
                (if group
                    (format popper-echo-prompt-group-format group)
                  popper-echo-prompt)
